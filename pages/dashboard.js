...
export default function Dashboard() {
  ...
  const [gradebookTab, setGradebookTab] = useState('assignments');
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-800 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6">
        ...
        {/* GRADEBOOK MAIN SECTION (CONTAINING THE GREEN TOPBAR) */}
        {activeTab === 'batchGradebook' && isStaffOrAdmin && (
          <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
            
            {/* Green Gradebook Sub-Navbar */}
            <div className="bg-[#a4cf53] px-6 py-2.5 flex items-center gap-8 shadow-inner border-b border-[#92bd44]">
              <button 
                onClick={() => setGradebookTab('home')}
                className={`text-sm font-semibold transition ${gradebookTab === 'home' ? 'text-white font-bold' : 'text-[#385507] hover:text-white'}`}>
                Home
              </button>
              <button 
                onClick={() => setGradebookTab('students')}
                className={`text-sm font-semibold transition ${gradebookTab === 'students' ? 'text-white font-bold' : 'text-[#385507] hover:text-white'}`}>
                Students
              </button>
              <button 
                onClick={() => setGradebookTab('classes')}
                className={`text-sm font-semibold transition ${gradebookTab === 'classes' ? 'text-white font-bold' : 'text-[#385507] hover:text-white'}`}>
                Classes
              </button>
              <button 
                onClick={() => setGradebookTab('assignments')}
                className={`text-sm font-semibold transition ${gradebookTab === 'assignments' ? 'text-white font-bold' : 'text-[#385507] hover:text-white'}`}>
                Assignments
              </button>
              <button 
                onClick={() => setGradebookTab('attendance')}
                className={`text-sm font-semibold transition ${gradebookTab === 'attendance' ? 'text-white font-bold' : 'text-[#385507] hover:text-white'}`}>
                Attendance
              </button>
            </div>

            {/* Inner Content Area based on Gradebook Sub-Tab */}
            <div className="p-6">
              
              {/* GRADEBOOK HOME SUB-TAB */}
              {gradebookTab === 'home' && (
                <div>
                  <h1 className="text-xl text-gray-800 font-normal mb-2">Gradebook Overview</h1>
                  <p className="text-xs text-gray-500">Select a section from the green navigation bar above to manage students, view classes, record assignment scores, or take attendance.</p>
                </div>
              )}

              {/* ASSIGNMENT SCORES GRID */}
              {gradebookTab === 'assignments' && (
                <div>
